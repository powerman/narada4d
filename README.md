...
[![Go version](https://img.shields.io/github/go-mod/go-version/powerman/narada4d?color=blue)](https://go.dev/)
[![Test](https://img.shields.io/github/actions/workflow/status/powerman/narada4d/test.yml?label=test)](https://github.com/powerman/narada4d/actions/workflows/test.yml)
[![Coverage Status](https://raw.githubusercontent.com/powerman/narada4d/gh-badges/coverage.svg)](https://github.com/powerman/narada4d/actions/workflows/test.yml)
[![Go Report Card](https://goreportcard.com/badge/github.com/powerman/narada4d)](https://goreportcard.com/report/github.com/powerman/narada4d)
[![Release](https://img.shields.io/github/v/release/powerman/narada4d?color=blue)](https://github.com/powerman/narada4d/releases/latest)
[![Go Reference](https://pkg.go.dev/badge/github.com/powerman/narada4d.svg)](https://pkg.go.dev/github.com/powerman/narada4d)
...
- Application's data schema has own versioning.
  - _Rationale:_ It's too easy to occasionally and sometimes makes sense
...
- Application shouldn't keep shared lock on data schema version for a
  ...
  - _Rationale:_ We need to get exclusive lock to make consistent backup
...
- Environment variable `$NARADA4D` must contains URL to data schema
  ...
  - _Rationale:_ This is convenient and flexible way to configure
...
- Environment variable `$NARADA4D_SKIP_LOCK` must be set to non-empty
  ...
  - _Rationale:_ Needed to support recursive locking, needed in case
...
- Version of data schema must be a string which is either `none` or
  ...
  (for ex. `42`, `0`, `0.12.0`).
  - _Rationale:_ This make it easier to use it as part of backup file
...
- Version of data schema must be set to `dirty` in case operation which
  ...
  - _Rationale:_ This prevent application from using damaged data
...
- Version of data schema must be set to `none` after initializing version
  ...
  - _Rationale:_ This enables to separate initialization of data schema
    version from other operations.

## Recommendations

- One application have only one data set and thus one data schema version.
  - _Rationale:_ This will ensure there will be no deadlocks while
...
- It usually doesn't makes any sense to keep application version and data
  ...
  - _Rationale:_ Data schema changes less often than application, so
...
- `.version`
  - Symlink to current data schema version.
    - _Rationale:_ Symlink can be read/written using one atomic
      syscall.
  ...
  - Modified only under exclusive lock on `.lock`.
    - _Rationale:_ This guarantee data schema version won't change
      while application hold shared or exclusive lock on `.lock`.
- `.lock` and `.lock.queue`
  - Usual, empty files.
  - Created while initialization.
  - Never removed.
    - _Rationale:_ This makes possible to open these files just once
  ...
  - Before trying to acquire shared or exclusive lock on `.lock` it's
    ...
    - _Rationale:_ It guarantee exclusive lock on `.lock` will be
      acquired ASAP.
...
- To check is it initialized: `SELECT COUNT(*) FROM Narada4D`.
- To set shared lock: `LOCK TABLE Narada4D READ`.
  - This won't prevent _anyone_ not aware about Narada4D (including
